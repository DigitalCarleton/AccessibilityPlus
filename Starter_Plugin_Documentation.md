...
# Conclusion
The database of a website stores the information that makes up the content of a website. Back end web developers use server-side languages and tools to allow the database to communicate with the server and the software application so that the content will be displayed on the front-end. When building plugins on Omeka–a content management system for digital archives– developers should work with Omeka's framework. This means following the plugin directory structure, using models, views and controllers, building forms with Omeka, and taking advantage of its many functions, listed in [OmekaDocs](https://omeka.org/classic/docs/). While it is possible to hardcode parts of plugins, such as forms, taking the time to learn the terminology and understand the Omeka framework will save you more time in the long run.
